...
@Service
@Slf4j
public class MaticTransactionGateway implements TransactionGateway {
    ...
    public Signature submit(final TransactionSignature signTransactionResponse,
        ...
        try {
            log.debug("Sending transaction to matic node {}", signTransactionResponse.getSignedTransaction());
            ...
            if (send.hasError()) {
                if (send.getError().getMessage().matches(".*[I,i]nsufficient funds.*")) {
                    log.warn("Got error from matic chain: insufficient funds");
                    throw arkaneException()
                            .errorCode("transaction.insufficient-funds")
                            .message("The account that initiated the transfer does not have enough energy")
                            .build();
                } else {
                    log.error("Got error from matic chain: {}", send.getError().getMessage());
                    throw arkaneException()
                            .errorCode("transaction.submit.matic-error")
                            .build();
                }
            } else {
            ...
            throw arkaneException()
                    ...
                    .message("A problem occurred trying to submit the transaction to the Matic network")
                    .cause(ex)
                    .build();
        }
    }

    private Web3j createWeb3j(final @Value("${network.arkane.matic.endpoint.url}") String endpoint) {
    ...
    private EthSendTransaction ethSendRawTransaction(final Web3j web3j,
        ...
        try {
            ...
            log.error("Problem trying to submit transaction to the Matic network: {}", ex.getMessage());
