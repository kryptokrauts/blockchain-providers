...
public class AeternityServiceAutoConfiguration {
    ...
    public TransactionService transactionService(final @Value("${network.arkane.aeternity.api.baseUrl}") String baseUrl,
    ...
    public ChainService chainService(final @Value("${network.arkane.aeternity.api.baseUrl}") String baseUrl) {
        return new ChainServiceFactory().getService(ServiceConfiguration.configure().baseUrl(baseUrl).compile());
    }
}
